(function() {
	...
	ScheduleTemplate.prototype.initEvents = function() {
		...
		for(var i = 0; i < this.singleEvents.length; i++) {
			...
			this.singleEvents[i].addEventListener('mouseover', function(event){
				event.preventDefault();
				console.log(self.animating);
		...
		this.modalInformation = this.element.getElementsByClassName('cd-schedule-modal__event-info')[0];
		
		this.modalInformation.addEventListener('mouseover', function(event){
			event.preventDefault();
			if( !self.animating ) self.closeModal();
		});
		
		
		this.coverLayer.addEventListener('mousemove', function(event){
			if( !self.animating ) self.closeModal();
		});
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		if( mq == 'mobile' ) {
			...
			self.modalHeaderBg.setAttribute('style', 'height: '+eventHeight+'px; transform: scaleY('+HeaderBgScaleY+')');
